...
import { View, Text,  FlatList, Pressable, TouchableOpacity, TextInput, } from "react-native";
...
const tabs = ["Favorites", "New", "USDC", "USDT", "FDUSD", "BNB", "BTC", "ALT"];
const CryptoMarketPopup: React.FC<Props> = ({ visible, onClose }) => {
    ...
    return (
        <Modal
            isVisible={visible}
            onBackdropPress={onClose}  // tap outside modal closes it
            onSwipeComplete={onClose}  // swipe down closes modal
            swipeDirection="down"      // enable swipe down to dismiss
        ...
        >
            <View className="flex-1 justify-end bg-black/40">
                <View className="bg-white rounded-t-2xl p-4 h-[80%]">
                    {/* Search Bar */}
                    <View className=" rounded-lg px-3 py-2 mb-3 flex-row items-center">
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-2">
                        {tabs.map((tab) => (
                            ...
                            >
                                ...
                                {activeTab === tab && <View className="h-1 w-4 bg-yellow-400 rounded-full mt-1" />}
                            </TouchableOpacity>
                        ))}
                    </ScrollView>
